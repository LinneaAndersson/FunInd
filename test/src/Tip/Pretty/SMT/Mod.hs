...
module Tip.Pretty.SMT.Mod where
...
ppFormula (Formula Prove attrs tvs term)  =
  apply "assert-not" $ par' tvs (ppExpr term)
ppFormula (Formula Assert attrs tvs term) =
  apply "assert" $ named $ par' tvs (ppExpr term)
    where named e = 
            case join $ lookup "name" attrs of
                Nothing     ->  e
                Just name   ->  " (! " <> e <> " :named " <> text name <> " )"
