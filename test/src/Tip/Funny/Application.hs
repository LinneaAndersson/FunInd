        ...
        allMatches <- zipWithM 
            (\l e -> 
                do 
                    ...
                    return (case expr of 
                        [] -> [ands (map (\(lhs',rhs') -> lhs' === rhs') (l:exprs))]
                        xs -> xs)) lhs rhs
...
gblExpr :: (PrettyVar a, Name a) => [(Expr a, Expr a)] -> Property a -> Expr a -> Fresh (Maybe (Expr a))
...
foldReqs list = do
    let exp = last list
    let i = init list
    exp' <- foldM (\e _ -> foldM (\e' l -> updateRef' [l] e' ) e (reverse i)) (fst exp === snd exp) i
    return exp' 
