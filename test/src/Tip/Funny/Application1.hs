module Tip.Funny.Application1 where
...
--        fail $ show $ ((ppExpr  fBody) : (map (ppExpr) outp)) ++ (map (\p' -> ppExpr (Gbl p' :@: [])) (propGlobals p))
        return outp
...
mExpr exprs p (Match m cs)        = 
    do
        ...
        allMatches <- zipWithM (\l e -> mExpr (l:exprs) p e) lhs rhs
...
gblExpr reqs p (Gbl g :@: ls) 
    | gbl_name g == func_name (propFunc p) = 
        do
            ...
            return $ Just (mkQuant Forall freeVars prop')
