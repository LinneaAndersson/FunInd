...
(assert-not
  :speculated-lemma
  (forall ((x Int) (y List1)) (= (app (Cons x Nil) y) (Cons x y))))
(assert-not
  :speculated-lemma
  (forall ((x List1)) (= (app x (app Nil Nil)) x)))
(assert-not
  :speculated-lemma
  (forall ((x List1)) (= (app (app Nil Nil) x) x)))
(assert-not
  :speculated-lemma
  (= (f1 TNil) Nil))
(assert-not
...
(assert-not
  :speculated-lemma
  (forall ((x TList)) (= (tapp x (tapp TNil TNil)) x)))
(assert-not
  :speculated-lemma
  (forall ((x TList)) (= (tapp (tapp TNil TNil) x) x)))
(check-sat)
