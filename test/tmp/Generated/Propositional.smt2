...
(assert-not
  :speculated-lemma
  (par (x)
    (forall ((y x) (z (list (pair x x))))
      (= (elem (pair2 y y) z) false))))
(assert-not
  :speculated-lemma
  (par (x)
    (forall ((y x) (z (list x)) (x2 (list (list x))))
      (= (elem (cons y z) x2) false))))
(assert-not
  :speculated-lemma
  (par (x y)
    (forall ((z x) (x2 y) (x3 (list (pair x y))))
      (= (elem (pair2 z x2) x3) false))))
(assert-not
  :speculated-lemma
  (par (x)
    (forall ((y x) (z (list x)) (x2 (list (list x))))
...
(assert-not
  :speculated-lemma
  (forall ((x Int)) (= (valid (Var x)) false)))
(assert-not
  :speculated-lemma
...
(assert-not
  :speculated-lemma
  (forall ((x Int)) (= (valid (Not (Var x))) false)))
(assert-not
  :speculated-lemma
...
(assert-not
  :speculated-lemma
  (forall ((x Form) (y Form)) (= (valid (|:&:| x (Not y))) false)))
(assert-not
  :speculated-lemma
  (par (x)
...
(assert-not
  ...
  (par (x)
    (forall ((y (list x)) (z (list (list x))))
      (= (elem (as nil (list x)) (cons (++ y y) z))
        (elem (as nil (list x)) (cons y (cons y z)))))))
(assert-not
  :speculated-lemma
  (par (x)
...
(assert-not
  ...
  (par (x)
    (forall
      ((y (list x)) (z (list x)) (x2 (list x)) (x3 (list (list x))))
      (= (elem y (cons (++ x2 z) x3)) (elem y (cons (++ z x2) x3))))))
(assert-not
  :speculated-lemma
  (par (x)
    (forall
      ((y (list x)) (z (list x)) (x2 (list x)) (x3 (list (list x))))
      (= (elem (++ y z) (cons y x3)) (elem (++ z x2) (cons x2 x3))))))
(assert-not
  :speculated-lemma
  (par (x)
    (forall
      ((y (list x)) (z (list x)) (x2 (list x)) (x3 (list (list x))))
      (= (elem (++ y (++ x2 z)) x3) (elem (++ y (++ z x2)) x3)))))
(check-sat)
